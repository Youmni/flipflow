...
const Cards = () => {
  ...
  const [isAddCardsetModalOpen, setIsAddCardsetModalOpen] = useState(false);
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [search, currentPage, userId, accessToken]);
  ...
  const handleAddCardClick = (id) => {
    navigate(`/cardset/${id}/edit`);
  };

  const handleAddCardsetClick = () => {
    setIsAddCardsetModalOpen(true);
  };

  return (
    <>
      <Helmet>
        <title>FlipFlow - My Cards</title>
      </Helmet>
      <div className="container mx-auto p-4">
        ...
        {/* Render de AddCardsetModal en geef isOpen en onClose door */}
        <AddCardsetModal
          isOpen={isAddCardsetModalOpen}
          onClose={() => setIsAddCardsetModalOpen(false)}
        />
      </div>
    </>
  );
